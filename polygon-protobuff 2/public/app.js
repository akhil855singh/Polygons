...
function loadPolygonsInView() {
    ...
    const minLat = Math.floor(sw.lat());  // Floored to 2 decimal places
    const minLng = Math.floor(sw.lng()); // Floored to 2 decimal places
    ...
    // Fetch polygons from the server only if not cached or bounds are different
    ...
    fetch(`/polygons?minLat=${minLat}&minLng=${minLng}&maxLat=${maxLat}&maxLng=${maxLng}`)
        ...
        .then(polygons => {
            ...
            // Update the lastBounds to the current bounds after the fetch
            lastBounds = { minLat, minLng, maxLat, maxLng };
